				...
				<div class="span9">
					...
					<div class="tab-content">
						<div class="tab-pane" id="listView">
							...
							<hr class="soft" />
							<%
								ArrayList<Products_container> prodlist = (ArrayList<Products_container>) session
										.getAttribute("prod_list");
								for (int i = 0; i < prodlist.size(); i++) {
									out.print("<div class=row><div class=span2><img src=themes/images/products/"
											+ prodlist.get(i).pic_id
											+ " alt= /></div><div class=span4><h3>"
											+ prodlist.get(i).p_name
											+ "</h3><hr class=soft/><form action=ProductShowServlet method=get><button type=submit name='pid' value="
											+ prodlist.get(i).p_id
											+ " value='pid' >View Details</a></form><br class=clr/></div><div class=span3 alignR> <form action=AddtoCart method=get><h3>&nbsp;Rs."
											+ prodlist.get(i).price
											+ "</h3>&nbsp; &nbsp;<button type=submit name='pid' value="
											+ prodlist.get(i).p_id
											+ "> Add to <i class=icon-shopping-cart></i></button></form></div></div><hr class=soft/>");
									//out.println("<input type='text' name='product_id' value="+prodlist.get(i).p_id+">");
								}
							%>
							...
							<ul class=thumbnails>
								<%
									for (int i = 0; i < prodlist.size(); i++) {
										...
										out.print("<li class=span3><div class=thumbnail><img src=themes/images/products/"
												+ prodlist.get(i).pic_id
												+ " alt=/><div class=caption><h5>"
												+ prodlist.get(i).p_name
												+ " </h5><p></p><h4 style=text-align:center><form action=ProductShowServlet method=get> <button type=submit name='pid' value="
												+ prodlist.get(i).p_id
												+ " value='pid'><i class=icon-zoom-in></i></button></form><form action=AddtoCart method=get><button type=submit name='pid' value="
												+ prodlist.get(i).p_id
												+ "> Add to <i class=icon-shopping-cart></i></button>&nbsp;<a class=btn href=#>Rs."
												+ prodlist.get(i).price
												+ "</a></h4></form></div></div></li>");
									}
								%>
